...
def process_choice(name):
    ...
    while True:
        choice = input('\nMake a choice else Enter to stast over or # to end: ')
        if choice == '':
            main()
        elif choice == '#':
            sys.exit()
    ...
    name = ''.join(left_over_list) # make diplay more readable
    return choice, name
...
def main():
    ...
    # remove hyphens in hephenated names 
    name = name.replace('-', '')
    ...
    while running:
        ...
        if len(temp_phrase) < limit:
            ...
            print("Currnet anagram phrase = ", end=" ")
